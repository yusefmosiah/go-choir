...
func (c *Client) Resolve(userID string) (*resolveResponse, error) {
	reqBody := resolveRequest{UserID: userID}
...
func (c *Client) Lookup(userID string) (*ownershipResponse, error) {
	endpoint := LookupEndpoint(c.baseURL) + "?user_id=" + userID
...
func (c *Client) Stop(userID string) error {
	return c.postAction(StopEndpoint(c.baseURL), userID)
}

// Remove requests vmctl to remove the ownership for the given user.
func (c *Client) Remove(userID string) error {
	return c.postAction(RemoveEndpoint(c.baseURL), userID)
}

// postAction sends a POST request with a user_id body to the given endpoint.
func (c *Client) postAction(endpoint, userID string) error {
	reqBody := resolveRequest{UserID: userID}
	data, err := json.Marshal(reqBody)
	if err != nil {
